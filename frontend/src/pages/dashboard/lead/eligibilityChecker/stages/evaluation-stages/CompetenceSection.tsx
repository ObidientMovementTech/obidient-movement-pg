...
interface CompetenceSectionProps {
  onNext: (data: { competenceScore: number }) => void;
}

const CompetenceSection: React.FC<CompetenceSectionProps> = ({ onNext }) => {
  const { title, sections }: EvaluationCategory = evaluationQuestions.competence;
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full bg-white rounded-xl p-5 sm:p-6 md:p-8 shadow-sm border border-gray-100">
      ...
      <div className="text-sm text-gray-600 mb-6 bg-blue-50 p-3 rounded-lg">
        <p>Assess the candidate's skills, knowledge, and expertise required for the position.</p>
      </div>

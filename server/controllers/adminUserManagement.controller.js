...
export const adminUserManagementController = {
  ...
  // Create new user - Enhanced for mass onboarding
  async createUser(req, res) {
    ...
    try {
      ...
      const {
        ...
        // System flags
        role = 'user',
        emailVerified = true, // Default true for admin-created users
        adminCreated = true,
        autoGenerateVotingBloc = true,
        countryOfResidence // Keep for backward compatibility
      } = req.body;
      ...
      // Auto-generate voting bloc if requested (after user is committed to database)
      let votingBlocCreated = false;
      if (autoGenerateVotingBloc) {
        try {
          // Clean up any existing duplicate auto voting blocs first
          await cleanupDuplicateAutoVotingBlocs(newUser.id);

          // Check if user already has an auto-generated voting bloc to prevent duplicates
          const existingBlocs = await VotingBloc.findByCreator(newUser.id);
          const existingAutoBloc = existingBlocs.find(bloc => bloc.isAutoGenerated);

          if (!existingAutoBloc) {
            // Create the user object with the new user data for the helper function
            const userForBloc = {
              id: newUser.id,
              email: newUser.email || email,
              name: newUser.name || name,
              votingState: votingState || null,
              votingLGA: votingLGA || null,
              votingWard: votingWard || null
            };

            await createAutoVotingBloc(userForBloc);
            console.log(`✅ Auto voting bloc created for user: ${newUser.email || email}`);
            votingBlocCreated = true;
          } else {
            console.log(`ℹ️  User ${newUser.email || email} already has an auto-generated voting bloc, skipping creation`);
          }
        } catch (votingBlocError) {
          console.error('Failed to create auto voting bloc:', votingBlocError);
          // Don't fail the entire user creation if voting bloc creation fails
        }
      }
      ...
      res.status(201).json({
        ...
        data: {
          ...newUser,
          hasAutoVotingBloc: votingBlocCreated
        }
      });
